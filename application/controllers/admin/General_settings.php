...
class General_settings extends MY_Controller {
	...
	public function index()
	{
		...
		$blogsettings = $this->setting_model->get_blog_settings();

		

		foreach($blogsettings as $metakey=>$metavalue){

			$data['blog_settings'][$metavalue['meta_key']] = $metavalue['meta_value'];
		}
	...
	public function add()
	{
		...
		$site_logo = $this->input->post('bloglogo');
		...
		if(!empty($_FILES['sitelogo']['name']))
		{
			...
			$result = $this->functions->file_insert($path, 'sitelogo', 'image', '197152');
			if($result['status'] == 1){
				$sitedata['sitelogo'] = $path.$result['msg'];
			}
			else{
				$this->session->set_flashdata('error', $result['msg']);
				redirect(base_url('admin/general_settings'), 'refresh');
		...
		$result = $this->setting_model->update_blog_general_setting($sitedata);
		if($result){
